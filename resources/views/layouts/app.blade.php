    ...
    <nav x-data="{ 
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex justify-between items-center h-16 transition-all duration-500"
                :class="scrolled || open ? 'h-16' : 'h-20'">
                <div class="flex items-center">
                    <a href="{{ route('home') }}" class="flex items-center gap-2 group">
                        <span class="text-2xl font-bold transition-colors duration-500"
                            :class="scrolled || open ? 'text-amber-600 dark:text-amber-500' : 'text-white'">Swarattive</span>
                    </a>
                </div>
                ...
                <div class="hidden lg:flex items-center space-x-8">
                    ...
                    @foreach($navLinks as $link)
                        <a href="{{ route($link['route']) }}" wire:navigate
                            class="transition-all duration-300 py-1" :class="{ 
                                    '{{ request()->routeIs($link['activePattern']) ? 'text-amber-600 font-semibold' : 'text-gray-700 dark:text-gray-300' }} hover:text-amber-600 dark:hover:text-amber-500': scrolled || open,
                                    '{{ request()->routeIs($link['activePattern']) ? 'text-white font-semibold' : 'text-white/80' }} hover:text-white': !scrolled && !open 
                                }">
                            {{ $link['label'] }}
                        </a>
                    @endforeach
                    ...
                    <div class="relative ml-2" x-data="{ showThemeMenu: false }">
                        ...
                        <div x-show="showThemeMenu" @click.outside="showThemeMenu = false"
                            ...
                            <button @click="setTheme('light'); showThemeMenu = false"
                                class="w-full text-left px-4 py-2 text-xs font-semibold hover:bg-amber-50 dark:hover:bg-amber-900/20 transition-colors flex items-center gap-2"
                                :class="theme === 'light' ? 'text-amber-600' : 'text-gray-700 dark:text-gray-300'">
                            ...
                            <button @click="setTheme('dark'); showThemeMenu = false"
                                class="w-full text-left px-4 py-2 text-xs font-semibold hover:bg-amber-50 dark:hover:bg-amber-900/20 transition-colors flex items-center gap-2"
                                :class="theme === 'dark' ? 'text-amber-600' : 'text-gray-700 dark:text-gray-300'">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                        d="M20.354 15.354A9 9 0 018.646 3.646 9.003 9.003 0 0012 21a9.003 9.003 0 008.354-5.646z">
                                    </path>
                            ...
                            <button @click="setTheme('system'); showThemeMenu = false"
                                class="w-full text-left px-4 py-2 text-xs font-semibold hover:bg-amber-50 dark:hover:bg-amber-900/20 transition-colors flex items-center gap-2"
                                :class="theme === 'system' ? 'text-amber-600' : 'text-gray-700 dark:text-gray-300'">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                        d="M9.75 17L9 20l-1 1h8l-1-1-.75-3M3 13h18M5 17h14a2 2 0 002-2V5a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z">
                                    </path>
                ...
                <div class="hidden md:flex lg:hidden items-center space-x-6">
                    <a href="{{ route('home') }}" wire:navigate class="transition-all duration-300 text-sm font-medium"
                        :class="scrolled || open ? 'text-gray-700 dark:text-gray-300' : 'text-white'">Beranda</a>
                    <a href="{{ route('services.index') }}" wire:navigate
                        class="transition-all duration-300 text-sm font-medium"
                        :class="scrolled || open ? 'text-gray-700 dark:text-gray-300' : 'text-white'">Layanan</a>
                    <a href="{{ route('booking.index') }}" wire:navigate
                ...
                <div class="lg:hidden">
                    <button @click="open = !open" class="p-2 transition-colors duration-300"
                        :class="scrolled || open ? 'text-gray-700 dark:text-gray-300' : 'text-white'">
        ...
        <div x-show="open" x-transition
            class="lg:hidden bg-white dark:bg-neutral-900 border-t dark:border-neutral-800 transition-colors duration-500">
                ...
                <div class="pt-4 pb-2 border-t border-gray-100 dark:border-neutral-800">
                    ...
                    <div class="flex items-center gap-2 px-3">
                        <button @click="setTheme('light')"
                            class="flex-1 flex flex-col items-center gap-1 p-2 rounded-xl transition-all"
                            :class="theme === 'light' ? 'bg-amber-600 text-white' : 'bg-gray-50 text-gray-700 dark:bg-neutral-800 dark:text-gray-300'">
                        ...
                        <button @click="setTheme('dark')"
                            class="flex-1 flex flex-col items-center gap-1 p-2 rounded-xl transition-all"
                            :class="theme === 'dark' ? 'bg-amber-600 text-white' : 'bg-gray-50 text-gray-700 dark:bg-neutral-800 dark:text-gray-300'">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                    d="M20.354 15.354A9 9 0 018.646 3.646 9.003 9.003 0 0012 21a9.003 9.003 0 008.354-5.646z">
                                </path>
                        ...
                        <button @click="setTheme('system')"
                            class="flex-1 flex flex-col items-center gap-1 p-2 rounded-xl transition-all"
                            :class="theme === 'system' ? 'bg-amber-600 text-white' : 'bg-gray-50 text-gray-700 dark:bg-neutral-800 dark:text-gray-300'">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                    d="M9.75 17L9 20l-1 1h8l-1-1-.75-3M3 13h18M5 17h14a2 2 0 002-2V5a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z">
                                </path>
