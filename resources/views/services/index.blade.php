...
@section('content')
<!-- Hero Section -->
<section class="relative h-[60vh] lg:h-[70vh] bg-cover bg-center" style="background-image: linear-gradient(rgba(0,0,0,0.6), rgba(0,0,0,0.6)), url('{{ $hero['image'] ? asset('storage/' . $hero['image']) : 'https://images.unsplash.com/photo-1554046920-90dc2c6b12a8?ixlib=rb-4.0.3&auto=format&fit=crop&w=1950&q=80' }}');">
                            ...
                            @foreach($categoryServices as $service)
                                <div class="bg-white rounded-2xl shadow-sm border border-[#ede8e3] overflow-hidden flex flex-col lg:flex-row transition-all hover:shadow-xl group">
                                    <div class="relative w-full lg:w-5/12 aspect-[4/3] lg:aspect-auto lg:h-auto overflow-hidden flex-shrink-0">
                                        <img src="{{ $service->image_url ? asset('storage/' . $service->image_url) : 'https://images.unsplash.com/photo-1519741497674-611481863552?auto=format&w=800&q=80' }}" alt="{{ $service->name }}" class="w-full h-full object-cover lg:absolute lg:inset-0 group-hover:scale-105 transition-transform duration-700 ease-in-out">
